...
const paymentSchema = zod_1.z.object({
    meanCode: zod_1.z.string().min(1),
    paymentNumber: zod_1.z.string().min(1),
    currency: zod_1.z.string().min(1),
    feesAmount: zod_1.z.number().min(0),
    amount: zod_1.z.number().min(1),
});
function validatePayment(req, res, next) {
    const parsed = paymentSchema.safeParse(Object.assign(Object.assign({}, req.body), { amount: req.body.amount ? Number(req.body.amount) : undefined, feesAmount: req.body.feesAmount ? Number(req.body.feesAmount) : undefined }));
    if (!parsed.success) {
