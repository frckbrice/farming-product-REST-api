...
const productSchema = zod_1.z.object({
    ...
    productCat: zod_1.z.string().min(1),
    priceType: zod_1.z.string().min(1),
    price: zod_1.z.number().min(0),
    description: zod_1.z.string().optional(),
    wholeSale: zod_1.z.boolean().optional(),
});
function validateProduct(req, res, next) {
    const data = req.body;
    // If using multer, req.body fields may be strings, so coerce as needed
    const parsed = productSchema.safeParse(Object.assign(Object.assign({}, data), { price: data.price ? Number(data.price) : undefined, wholeSale: data.wholeSale === "true" || data.wholeSale === true }));
    if (!parsed.success) {
 ...
 */
productRouter.post("/add", (0, multerStorage_1.default)("productImage"), validateProduct, product_controller_1.createProduct);
/**
 * @swagger
