...
const orderSchema = zod_1.z.object({
    shipAddress: zod_1.z.string().min(1),
    weight: zod_1.z.string().min(1),
    sellerId: zod_1.z.string().uuid(),
    amount: zod_1.z.number().min(1),
});
function validateOrder(req, res, next) {
    const parsed = orderSchema.safeParse(Object.assign(Object.assign({}, req.body), { amount: req.body.amount ? Number(req.body.amount) : undefined }));
    if (!parsed.success) {
 ...
 */
orderRouter.post("/new/:productId", validateOrder, order_controller_1.createOrder);
/**
 * @swagger
